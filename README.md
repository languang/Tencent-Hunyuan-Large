...
#### inference Framework
- This open-source release provides the inference backend tailored for the Hunyuan-Large model, based on the vLLM open-source framework, [vLLM-backend](https://github.com/quinnrong94/vllm/tree/dev_hunyuan) , with optimizations for enhanced performance, which for example include the introduction of a new CLA structure that significantly reduces GPU memory usage (with a 50% savings in the KV-Cache portion), ensuring efficient handling of long text scenarios. Additionally, through FP8 quantization optimization, we achieve a 50% reduction in memory usage compared to conventional FP16/BF16 quantization while maintaining precision, resulting in a 70% increase in throughput.
...
HunyuanLLM uses vLLM for deployment. To simplify the deployment process, HunyuanLLM provides a pre-built Docker image. See the section [[#Using vLLM for Inference]].
